...
siemensTCP_Function_t	siemensTCP_ReceiveFunction = NULL;
siemensTCP_Function_t	siemensTCP_TransmitFunction = NULL;
...
{
	if( uip_conn->lport != HTONS( SIEMENS_TCP_PORT ) ||
	    !siemensTCP_ReceiveFunction || !siemensTCP_ReceiveFunction ) return;
...
//						memcpy( Request.data, data, Request.num_bytes );
						...
						if( siemensTCP_ReceiveFunction )
							siemensTCP_ReceiveFunction( &tr );
					}
					// Next write data
...
//			memcpy( Request.data, tr.data, Request.num_bytes );
			...
			// Do actual data transfer
			if( siemensTCP_TransmitFunction )
				siemensTCP_TransmitFunction( &tr );
...
void siemensTCP_set_receive_function(siemensTCP_Function_t func)
{
	siemensTCP_ReceiveFunction = func;
}
void siemensTCP_set_transmit_function(siemensTCP_Function_t func)
{
	siemensTCP_TransmitFunction = func;
}
