 ...
 */
static udp_frame_callback_t	udp_frame_callback = NULL;
...
void udp_frame_appcall(void)
{
	...
	if( udp_frame_callback != NULL && uip_newdata() ) {
		udp_frame_callback( uip_udp_conn->ripaddr, uip_udp_conn->rport,
							uip_appdata, uip_len );
		docheck = 1;
	}
	if( uip_poll() || docheck ) {
 ...
 * The packet data length is limited to 1500 bytes (approx.)
 */
...
int	udp_frame_send(void* ip, int port, char* data, int len)
{
	...
	i = (conn - uip_udp_conns)/sizeof(uip_udp_conns[0]);
