...
//!
//! @note Because the OLED display packs 2 pixels of data in a single byte, the
...
//*****************************************************************************
void
RIT128x96x4ImageDraw(const unsigned char *pucImage, int x,
                     int y, int width,
                     int height)
{
    //
    ...
    x >>= 1; width >>= 1;
    if( x < 0 ) {
    	rX = 0; rWidth = width - (rX - x); pucImage += (rX - x);
    }else{
    	rX = x; rWidth = width;
    }
    if( (rX + rWidth) >= 128/2 ) {
    ...
    if( rWidth < 0 ) return;
    if( y < 0 ) {
    	rY = 0; height -= (rY - y); pucImage += (rY - y)*width;
    }else{
    	rY = y;
    }
    if( (rY + height) >= 96 ) {
    ...
    g_pucBuffer[0] = 0x15;
    g_pucBuffer[1] = rX;
    g_pucBuffer[2] = rX + rWidth - 1;
    RITWriteCommand(g_pucBuffer, 3);
    g_pucBuffer[0] = 0x75;
    g_pucBuffer[1] = rY;
    g_pucBuffer[2] = rY + height - 1;
    RITWriteCommand(g_pucBuffer, 3);
    RITWriteCommand(g_pucRIT128x96x4HorizontalInc,
                    sizeof(g_pucRIT128x96x4HorizontalInc));
