...
#define	LIBMU_INTERNET_STACK_SIZE	1024
	...
	/* Spawn the task. */
	xTaskCreate( libMU_Ethernet_Task, "Ethernet", LIBMU_INTERNET_STACK_SIZE,
 ...
 *   happens is when there is not enough memory
...
{
	/* Setup the FreeRTOS interrupt vectors */
...
{
	/* Spawn the task. */
	xTaskCreate( task, "Helper", LIBMU_INTERNET_STACK_SIZE,
				 NULL, tskIDLE_PRIORITY, ( xTaskHandle * ) NULL );
}
...
{
	char* p = strstr( cmd_params, param_tag );
	result_size--; /* Leave space for string ending character '\0' */
	if( p != NULL && ( p[-1]=='&' || p == cmd_params ) ) {
