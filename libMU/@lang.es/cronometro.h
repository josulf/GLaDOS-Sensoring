...
/**
 * Constantes usadas en la gestion del cronómetro
 */
enum libMU_Stopwatch_Constants {
	libMU_Stopwatch_FREQ		= 50000000,		/* Frecuencia de contaje 50MHz */
	libMU_Stopwatch_MAX_COUNT	= 0x80000000	/* Cuenta maxima de HW = (20ns * 2^31) ~ 42,95s */
};
...
/**
 * Tiempo intermedio el cronometraje (en ms)
 * @param	tiempo	Dirección de la variable donde se va a guardar el tiempo intermedio
 * @return			Devuelve el tiempo transcurrido desde la anterior medida de tiempo en ms
 */
static inline libMU_Time_t	libMU_Stopwatch_Peek_ms(libMU_Stopwatch_Time_t* tiempo)
{
	return libMU_Stopwatch_Peek( tiempo ) / ( libMU_Stopwatch_FREQ * 2 / 1000 );
}
