...
static const pwmc_t PWMC[ LIBMU_PWM_NUMBER ] = {
...
int	libMU_PWM_InitializeOutput( libMU_PWM_Constants_t outputID, uint32_t freq )
{
	...
	/*PWMSyncUpdate	( PWM_BASE, PWMC[outputID].gen_bit );*/
 ...
 * @note
 * - The real resolution is 100*Fpwm/Fcpu (in percentage)
 * - If the PWM frequency is above 400kHz this function does not work, use
 *   the function libMU_PWM_SetPulseWidthPPM() in that case
...
void	libMU_PWM_SetPulseWidth( libMU_PWM_Constants_t outId, uint32_t pulsewidth )
{
	...
	pw *= PWMGenPeriodGet( PWM_BASE, PWMC[outId].gen_code );
	pw += (scale>>1);	/* Round to closest integer when dividing */
	pw /= scale;
 ...
 * Set-up PWM output pulse width
 * @param	outId		Output identifier (single output)
 * @param	pulsewidth	In parts per billion (PPB): 0 -> 0%, 1000000000 -> 100%, 456789012 -> 45.6789012%
 * @note
 * - The real resolution is 100*Fpwm/Fcpu (in percentage)
 */
void	libMU_PWM_SetPulseWidthPPB( libMU_PWM_Constants_t outId, uint32_t pulsewidth )
{
	const uint32_t	scale = 1000000000;
	uint64_t pw = pulsewidth;
	pw *= PWMGenPeriodGet( PWM_BASE, PWMC[outId].gen_code );
	pw += (scale>>1);	/* Round to closest integer when dividing */
	pw /= scale;
	if( pw == 0 ) pulsewidth = 1;
	else		  pulsewidth = pw;
	PWMPulseWidthSet( PWM_BASE, PWMC[outId].out_code, pulsewidth );
}
...
{
	PWMOutputState( PWM_BASE, PWMC[outId].out_bit, false );
	GPIOPinTypeGPIOOutput( PWMC[outId].gpio_port, PWMC[outId].gpio_pin );
}
...
{
	GPIOPinTypePWM( PWMC[outId].gpio_port, PWMC[outId].gpio_pin );
	PWMOutputState( PWM_BASE, PWMC[outId].out_bit, true );
}

/**
 * Disable PWM peripheral
 */
void	libMU_PWM_Disable( void )
{
	SysCtlPeripheralReset( SYSCTL_PERIPH_PWM );
	SysCtlPeripheralDisable	( SYSCTL_PERIPH_PWM );	/* Disable PWM peripheral */
}
