...
/* Datos de mi imagen */
#define	AREA_VERTICAL		(libMU_Display_RES_Y - libMU_Display_CHAR_HEIGHT)

#define	MI_IMAGEN_X			32
#define	MI_IMAGEN_Y			7

uint8_t						mi_imagen_buffer[ MI_IMAGEN_X * libMU_Display_BPP / 8 * MI_IMAGEN_Y ];
...
void main_demo_06(void)
{
    ...
    libMU_Display_MemoryImage_InitializeLocal( libMU_Display_RES_X, AREA_VERTICAL );
	...
	for(;;)
	{
		...
		/* Gestiona el cambio de animación */
		if( cambia ) {
			cambia = 0;
			if( animacion_lineas ) animacion_lineas = 0; else animacion_lineas = 1;
			/* Inicializa la animación principal */
			if( animacion_lineas ) {
				libMU_Display_DrawString	( "SEL>Ellip", 0, 0, 12 );
				Baile_de_elipses_inicializa();	retardo = 10;
			}else{
				libMU_Display_DrawString	( "SEL>Lines", 0, 0, 12 );
				Baile_de_lineas_inicializa();	retardo = 10;
			}
		}
		/* Animación principal */
		if( animacion_lineas )	Baile_de_elipses_mueve();
		else					Baile_de_lineas_mueve();
		/* Animación de la linea superior */
		Baile_de_mi_imagen();
		/* Mostrar temperatura */
		CheckTemperatureXY( 64,0 );
		/* Espera */
		libMU_Delay( retardo );
		/* Lee si se ha pedido un cambio de animación */
		cambia = libMU_Button_Pressed( BUTTON_ALL );
	}	
}
...
void Baile_de_lineas_mueve(void)
{
	...
	if(points_X1[29] >= libMU_Display_RES_X )
	{
		points_X1[29] = libMU_Display_RES_X - 1;
	...
	if(points_X2[29] >= libMU_Display_RES_X )
	{
		points_X2[29] = libMU_Display_RES_X - 1;
	...
	for(ulLoop = 0; ulLoop < 30; ulLoop++)
	{
		...
		{
			libMU_Display_MemoryImage_DrawLineLocal(
					points_X1[ulLoop], points_Y1[ulLoop],
					points_X2[ulLoop], points_Y2[ulLoop],
					(ulLoop / 2) + 1);
		}
	}
...
void Baile_de_elipses_inicializa(void)
{
    ...
    memset( points_X2,  0, sizeof(points_Y1) );
    memset( points_Y2,  0, sizeof(points_X2) );
    /* Choose starting points for the first ellipse. */
    points_X1[29] = 0;					points_Y1[29] = 0;
    points_X2[29] = libMU_Display_RES_X - 1;	points_Y2[29] = AREA_VERTICAL - 1;
...
void Baile_de_elipses_mueve(void)
{
	...
	for(ulLoop = 0; ulLoop < 30; ulLoop++)
	{
		...
		if(points_X1[ulLoop] || points_Y1[ulLoop] ||
		   points_X2[ulLoop] || points_Y2[ulLoop])
		{
			libMU_Display_MemoryImage_DrawEllipse(
					&libMU_Display_LocalImage,
					points_X1[ulLoop], points_Y1[ulLoop],
					points_X2[ulLoop], points_Y2[ulLoop],
					(ulLoop / 2) + 1);
		}
