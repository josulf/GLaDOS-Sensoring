...
static __inline uint8_t bit_a_texto( uint32_t num, uint8_t bit)
{
	return (num & (1u<<bit))?'1':'0';
#if 0
	unsigned long mask = (1u<<bit);		/* All zeros except one '1' in the position given by 'bit' */
...
void main_demo_01(void)
{
	...
	for(;;)	/* repite indefinidamente */
	{
		...
		/* Mostrar resultados */
        libMU_snprintf( msg, sizeof(msg),
        				"Botones = %c%c%c%c|%c%c%c%c",
